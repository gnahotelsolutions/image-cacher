...
class ImageMagick extends Manager
{
    ...
    public function process($imageResource, int $width, int $height, array $originalDimensions, bool $crop = false, int $sharpen = 0)
    {
        try {
            ...
            if ($crop) {
                $cropGeometry = $this->calculateCropGeometry(
    ...
    protected function calculateCropGeometry(int $srcWidth, int $srcHeight, int $destWidth, int $destHeight): array
    {
        $aspectRatio = $destWidth / $destHeight;

        $cropWidth = round($srcHeight * $aspectRatio);
